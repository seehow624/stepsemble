...
const { WIRE_VERSION, LIMITS, uuid, keys, detach, validRequest, validPage, readResponse, sourceVersion } = require("./history-worker-wire");
...
function createSourceService({ spawnChild = spawn, platform = process.platform,
  ...
  function bind(input) {
    ...
    function revoke() {
      state.revoked = true; state.version = null; state.flight?.stop("source_binding_revoked");
    }
    async function capture(input, { signal } = {}, page = null, version = null) {
      ...
      // Allocate before launch, not in the synchronous ChildProcess close
      // listener: an entropy failure must not become an uncaught event error.
      const nextVersionToken = page && !version ? crypto.randomBytes(32).toString("hex") : null;
      if (page) job.history = { sdkPath, page, expectedVersion: version?.source ?? null };
      ...
      try {
        ...
        child.once("close", (code, signalName) => {
          ...
          if (result.kind === "source_unavailable") {
            // Once an observed change invalidates a version it cannot revive,
            // even if the source later happens to regain the same bytes.
            if (result.code === "source_version_changed" && state.version === version) state.version = null;
            return settle(result);
          }
          if (page) {
            // One small version per binding. Never retain history rows or an
            // unbounded cursor map. A successful unversioned read starts anew;
            // failures/cancellation cannot replace a previous working version.
            if (!version) state.version = { token: nextVersionToken, source: sourceVersion(result.source) };
            return settle({ kind: "bound_history_observation", ...request, sourceVersion: state.version.token, history: result,
    ...
    async function observe(input, options = {}) {
      if (!options || typeof options !== "object" || Array.isArray(options)
        ...
        || Object.entries(Object.getOwnPropertyDescriptors(options)).some(([key, d]) => !["page", "signal", "version"].includes(key) || !Object.hasOwn(d, "value")))
