...
function packet(metadata = body(), payload = Buffer.from(JSON.stringify(metadata))) {
  const r = request();
  return { payload, header: { kind: "native_sqlite_metadata", nativeVersion: r.nativeVersion, threadId: id, expectedRoot: r.expectedRoot,
...
function frame(job, p = packet(), change = v => v) {
  const header = Buffer.from(JSON.stringify(change({ protocolVersion: 4, nonce: job.nonce, result: p.header }))), size = Buffer.alloc(4);
...
module.exports = { id, request, body, packet, frame, capture };
