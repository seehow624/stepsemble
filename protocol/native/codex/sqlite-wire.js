...
const LIMITS = Object.freeze({ inputBytes: 12 * 1024, payloadBytes: 144 * 1024, textBytes: 32 * 1024,
  outputBytes: 4 + 16 * 1024 + 144 * 1024, readBytes: 8 * 1024 * 1024, readCalls: 1024, mapBytes: 8 * 1024 * 1024, maps: 256 });
...
function body(v, id) {
  ...
  return keys(o, ["kind", "nativeVersion", "sqliteVersion", "scope", "fields", "nativeTitleResolved", "sourceAuthenticated", "publishable", "connectionClosed"])
    && o.kind === "codex_sqlite_metadata_observation" && o.nativeVersion === VERSION && o.sqliteVersion === SQLITE_VERSION
    && o.scope === "provided_connection_selected_name_fields_only" && fields(o.fields, id) && o.nativeTitleResolved === false
...
function header(v, job) {
  return keys(v, ["kind", "nativeVersion", "threadId", "expectedRoot", "byteLength", "sha256", "sourceAuthenticated", "publishable"])
    && v.kind === "native_sqlite_metadata" && v.nativeVersion === VERSION && v.nativeVersion === job.nativeVersion
...
function sourceVersion(value, request) {
  ...
  return { kind: "codex_sqlite_selected_fields_version", nativeVersion: VERSION, threadId: v.threadId,
    rootIdentity: v.expectedRoot, identities: v.metadata.identities, fieldsSha256: digest(canonicalJSON(v.metadata.observation.fields)) };
}
function validVersion(v) {
  return keys(v, ["kind", "nativeVersion", "threadId", "rootIdentity", "identities", "fieldsSha256"])
    && v.kind === "codex_sqlite_selected_fields_version" && v.nativeVersion === VERSION && uuid(v.threadId)
...
module.exports = { VERSION, SQLITE_VERSION, LIMITS, keys, own, detach, input, decode, capture, sourceVersion, sameSourceVersion };
