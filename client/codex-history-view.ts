...
declare function require(name: "./agent-identity"): typeof StepsembleAgentIdentity;
...
namespace StepsembleCodexHistoryView {
  ...
  export function createModel(deps: Dependencies) {
    ...
    async function release(row: Registration | null): Promise<boolean> {
      if (!row) return false;
      try { const result = await deps.transport.release({ bindingId: row.bindingId, generation: row.generation });
        if (result.kind === "history_released" && result.cleanupConfirmed) return true;
        cleanupPending = true;
    ...
    async function load(mode: "refresh" | "next" | "previous" | "jump", target?: number, interruptPrevious = true) {
      if (!selected || closed || mode !== "refresh" && (busy || localAbortPending)) return;
    ...
    function cancel() {
      epoch++;
      if (flight && !flight.controller.signal.aborted) { localAbortPending = true; flight.controller.abort(); }
      busy = false; stage = "cancelled"; notify();
    }
  ...
  export function create(deps: Dependencies & { root: HTMLElement }) {
    ...
    const modeButton = (value: boolean) => {
      const v = copy("button", value ? "codexReadable" : "codexRawMode"); v.type = "button"; v.dataset.historyMode = value ? "structured" : "raw";
      v.addEventListener("click", () => { void model.setStructured(value); }); modes.append(v); return v;
    };
    ...
    function render() {
      ...
      jumpForm.hidden = !r; jumpInput.disabled = jumpButton.disabled = !s.canJump;
      ...
      for (const row of r.records) {
        ...
        if (turn && turn.turnKey !== priorTurn) {
          const section = el("header", "", "codex-history-turn"); section.dataset.turnKey = turn.turnKey; section.dataset.branchState = turn.branchState;
          const label = copy("h4", turn.boundary === "explicit" ? "codexTurn" : "codexInferredTurn");
          i18n.bind(label, turn.boundary === "explicit" ? "codexTurn" : "codexInferredTurn", { record: turn.firstRecordIndex + 1 }); section.append(label);
          if (turn.nativeTurnId !== null) section.append(el("p", turn.nativeTurnId, "history-footnote"));
          const statusLabel = el("p", "", "history-footnote");
          const statuses = { unknown: "codexStatusUnknown", started: "codexStatusStarted", completed: "codexStatusCompleted", failed: "codexStatusFailed", interrupted: "codexStatusInterrupted" } as const;
          statusLabel.append(copy("span", "codexRecordedStatus"), el("span", " · "), copy("span", statuses[turn.recordedStatus])); section.append(statusLabel);
          if (turn.branchState === "rolled_back") section.append(copy("p", "codexRolledBack", "history-warning"));
        ...
        if (annotation) {
          article.dataset.recordKind = annotation.kind; article.dataset.branchState = turn?.branchState ?? "retained";
          const label = el("span");
          if (annotation.kind === "assistant") { heading.replaceChildren(identity.create(doc, "codex", true), el("span", "Codex")); }
          else {
            const labels: Record<string, StepsembleHistoryI18n.Key> = { user: "you", reasoning: "thinking", tool: annotation.tool?.phase === "end" ? "toolResult" : "toolUse", model_context: "codexModelContext", metadata: "codexMetadata", lifecycle: "system" };
            i18n.bind(label, labels[annotation.kind] ?? "other"); heading.replaceChildren(label);
          }
        ...
        if (annotation?.tool) {
          ...
          if (related !== null) {
            const reference = copy("button", "codexRelatedRecord"); i18n.bind(reference, "codexRelatedRecord", { record: related + 1 }); reference.type = "button";
            reference.dataset.relatedRecord = String(related); reference.disabled = !s.canJump; relatedButtons.push(reference);
            reference.addEventListener("click", () => { void model.jump(related).then(() => {
              if (model.state().page?.history.records.offset !== related) return;
              const target = content.querySelector<HTMLElement>(`#codex-record-${related}`); if (target) { target.tabIndex = -1; target.focus(); }
