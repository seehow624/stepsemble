...
(() => {
  ...
  const AGENT_TERMINAL_KEYED = {
    ...
    "quotaSources.note": ["Limits come from each agent's own sign-in and from quota services on the selected host, such as OpenCodex.","額度來自各 Agent 自己的登入，以及所選主機上的額度服務，例如 OpenCodex。","额度来自各 Agent 自己的登录，以及所选主机上的额度服务，例如 OpenCodex。","利用枠は各エージェント自身のサインインと、選択中のホストにある OpenCodex などの利用枠サービスから取得します。","한도는 각 에이전트 자체 로그인과 선택한 호스트의 OpenCodex 같은 한도 서비스에서 가져옵니다.","Limitler her ajanın kendi oturumundan ve seçili ana makinedeki OpenCodex gibi kota hizmetlerinden gelir.","Les limites proviennent de la connexion de chaque agent et des services de quotas de l’hôte choisi, comme OpenCodex.","Die Limits stammen aus der eigenen Anmeldung jedes Agenten und aus Kontingentdiensten auf dem gewählten Host, etwa OpenCodex.","Los límites vienen del inicio de sesión de cada agente y de servicios de cuota del equipo elegido, como OpenCodex.","Os limites vêm do login de cada agente e de serviços de cota no host escolhido, como o OpenCodex.","I limiti arrivano dall’accesso di ogni agent e dai servizi di quota sull’host scelto, come OpenCodex."],
