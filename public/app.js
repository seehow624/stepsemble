/* pi-web v1.11.8 — English-first localization and provider catalog */
"use strict";

// ===========================================================================
// 機器清單（server 端權威下發）＋ 設定（localStorage 只記選擇與偏好）
// ===========================================================================
...
function saveSelected(id) { try { localStorage.setItem(SELECTED_KEY, id); } catch {} }

function currentMachine() {
  return machines.find(m => m.id === selectedId) || machines.find(m => m.self) || machines[0] || null;
}
...
function machineName(id) {
  ...
  return m ? machineDisplayName(m) : id;
}

function loadSettings() {
  try {
    const v2 = localStorage.getItem(SETTINGS_KEY);
    const raw = v2 || localStorage.getItem(LEGACY_SETTINGS_KEY) || "{}";
    ...
    out.locale = window.piI18n?.normalizeLocale(out.locale) || "en";
