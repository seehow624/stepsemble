...
(function (StepsembleHistoryView) {
    ...
    function createModel(deps) {
        ...
        async function load(mode, selectionEpoch) {
            ...
            try {
                ...
                if (ticket !== epoch || abort.signal.aborted)
                    return;
                error = object(reason) && typeof reason.code === "string" && Object.hasOwn(messages, reason.code) ? reason.code : "history_transport_failed";
                stale = snapshot.pages.length > 0;
            }
            finally {
                if (target !== controller)
        ...
        function cancel() { cancelWork(); stage = "已取消讀取；原有頁面仍保留"; notify(); }
    ...
    function create(deps) {
        ...
        const button = (key, label, action, parent = toolbar) => {
        ...
        const tabButtons = deps.catalog.map(entry => {
            const node = button("source:" + entry.catalogId, entry.label, () => model.select(entry.catalogId), tabs);
            node.setAttribute("aria-pressed", "false");
            return node;
        });
        button("refresh", "重新整理", () => model.refresh());
        ...
        for (const n of [5, 10, 25]) {
            const option = element("option", n + " 則");
        ...
        content.setAttribute("aria-label", "歷史訊息");
        const position = element("p", "", "history-position"), windowBar = element("div", "", "history-window-controls");
        button("windowPrevious", "本頁前 10 則", () => model.showWindow(Math.max(0, model.state().messageStart - StepsembleHistoryView.LIMITS.messages)), windowBar);
        button("windowNext", "本頁後 10 則", () => model.showWindow(model.state().messageStart + StepsembleHistoryView.LIMITS.messages), windowBar);
        ...
        function render() {
            ...
            const details = (heading, value, parent) => {
            ...
            if (!pageMessages.length)
                content.append(element("p", "這一頁沒有訊息。", "history-empty"));
            for (const message of pageMessages) {
                ...
                for (const block of blocks.slice(0, StepsembleHistoryView.LIMITS.blocks)) {
                    ...
                    if (block.kind === "text" && typeof block.text === "string")
                        ...
                        details("思考紀錄 · 唯讀", block.text, card);
                    else if (block.kind === "tool_use")
                        details("工具請求紀錄 · 不執行", block, card);
                    else if (block.kind === "tool_result")
                        details("工具結果紀錄 · 不代表已核准", block, card);
                    else if (block.kind === "attachment")
                        details("附件描述 · 不載入附件", block, card);
                    else
                        details("其他來源紀錄 · 唯讀", block, card);
                }
                if (blocks.length > StepsembleHistoryView.LIMITS.blocks)
                    card.append(element("p", "此訊息只顯示前 24 個內容區塊。", "history-footnote"));
                content.append(card);
            }
            for (const [key, heading] of [["tools", "本頁工具觀測 · 沒有執行權限"], ["auxiliaryRecords", "附加來源紀錄 · 不載入檔案"], ["warnings", "來源觀測限制"]]) {
                ...
                if (Array.isArray(rows) && rows.length)
                    details(`${heading}（${rows.length} 筆，最多顯示 ${StepsembleHistoryView.LIMITS.evidence} 筆）`, rows.slice(0, StepsembleHistoryView.LIMITS.evidence), evidence);
            }
        }
        render();
    ...
    async function bootHost() {
        ...
        try {
            ...
            if (groups.kind === "source_unavailable" && !["history_method_not_allowed", "history_source_unavailable"].includes(groups.code)) {
                showUnavailable(describeError(groups.code));
                return;
            }
            ...
            if (result.entries.length) {
                ...
                if (sources.length) {
                    ...
                    summary.textContent = `單獨登記的來源（${result.entries.length}）`;
    ...
    async function bootPreview() {
        ...
        try {
            ...
            if (result.kind !== "history_catalog") {
                status.textContent = describeError(result.code);
                return;
            }
            const provider = StepsembleClaudeHistory.create({ canonicalJSON });
            ...
            status.textContent = "這個頁面需要隔離的開發預覽主機。無法載入範例來源。";
        }
    }
