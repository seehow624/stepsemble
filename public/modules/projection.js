...
var StepsembleProjection;
(function (StepsembleProjection) {
    ...
    const unsettled = (a) => a.approval.status === "pending" || ["approved", "denied"].includes(a.approval.status) && a.nativeAcknowledgement === null;
    const instant = (time) => time.length <= 64 && !/\.\d{4}/.test(time) ? Date.parse(time) : NaN;
    const sameCursor = (a, b) => a.sessionId === b.sessionId && a.generation === b.generation && a.sequence === b.sequence;
    const sameProfile = (a, b) => ["launchProfileId", "harnessId", "modelId", "sourceId", "authMode", "billingMode", "credentialReference"].every(key => a[key] === b[key]);
    ...
    function canonicalJSON(value, limit = LIMIT) {
        let nodes = 0, units = 0;
        const seen = new Set();
        function visit(item, depth) {
            ...
            if (typeof item === "string") {
                units += item.length;
                if (units > limit || !wellFormed(item))
                    throw 0;
                return JSON.stringify(item);
            }
            ...
            for (const key of keys) {
                if (array && key === "length")
                    continue;
                if (typeof key !== "string" || !wellFormed(key) || !("value" in descriptors[key]) || !descriptors[key].enumerable)
                    throw 0;
                units += key.length;
                if (units > limit)
                    throw 0;
        ...
        try {
            ...
            return new TextEncoder().encode(json).length <= limit ? json : null;
        }
        catch {
            return null;
        }
    }
    StepsembleProjection.canonicalJSON = canonicalJSON;
    async function hash(kind, canonical) {
        const bytes = new TextEncoder().encode(`["stepsemble.${kind}.v1",${canonical}]`);
        const result = await globalThis.crypto.subtle.digest("SHA-256", bytes);
        return Array.from(new Uint8Array(result), byte => byte.toString(16).padStart(2, "0")).join("");
    }
    function create(contracts, lifecycle) {
        const rows = (s) => [s.runs, s.approvals, s.messages, s.tools, s.usage, s.contexts];
        function checkState(input) {
            if (canonicalJSON(input) === null || !contracts.validate("sessionProjection", input).valid)
                return bad("invalid_payload");
            return checkRelations(input);
        }
        function checkRelations(s) {
            const { cursor } = s, end = s.updatedAt === null ? NaN : instant(s.updatedAt);
            if (s.identityFloor > cursor.sequence || cursor.sequence - s.identityFloor !== s.identities.length)
                return bad("identity_conflict");
            const ids = new Set();
            for (let i = 0; i < s.identities.length; i++) {
                const id = s.identities[i];
                if (id.sequence !== s.identityFloor + i + 1 || ids.has(id.eventId))
                    return bad("identity_conflict");
                ids.add(id.eventId);
            }
            if (cursor.sequence === 0)
                return s.session === null && s.updatedAt === null && rows(s).every(list => !list.length) ? { valid: true } : bad("session_conflict");
            if (!Number.isFinite(end) || s.session === null || !lifecycle.checkSession(s.session).valid || s.session.session.sessionId !== cursor.sessionId)
                return bad("session_conflict");
            const start = instant(s.session.session.createdAt);
            const time = (created, updated) => instant(created) >= start && instant(updated) >= instant(created) && instant(updated) <= end;
            if (!time(s.session.session.createdAt, s.session.updatedAt) || s.session.revision >= cursor.sequence)
                return bad("state_time_conflict");
            const runs = new Map();
            let writer = null;
            for (const row of s.runs) {
                if (!lifecycle.checkRun(row).valid || row.run.sessionId !== cursor.sessionId || runs.has(row.run.runId) || row.revision >= cursor.sequence
                    || row.launchProfile.harnessId !== s.session.session.native.harnessId || !time(row.run.createdAt, row.updatedAt))
                    return bad("run_conflict");
                if (!terminal(row.run.state)) {
                    if (writer || s.session.session.status !== "active" || !s.session.launchProfile || !sameProfile(s.session.launchProfile, row.launchProfile))
                        return bad("writer_conflict");
                    writer = row;
                }
                runs.set(row.run.runId, row);
            }
            const bound = (row, created = row.updatedAt) => {
                const run = runs.get(row.runId);
                return run && time(created, row.updatedAt) && instant(created) >= instant(run.run.createdAt) ? run : null;
            };
            const toolMap = new Map(), messageIds = new Set();
            for (const m of s.messages) {
                ...
                if (!run || messageIds.has(m.messageId) || m.role === "user" && (m.status !== "completed" || m.thinking !== "")
                    ...
                    || m.role === "assistant" && (run.startedAt === null || instant(m.createdAt) < instant(run.startedAt)))
                    return bad("message_conflict");
            ...
            for (const t of s.tools) {
                ...
                if (!run || toolMap.has(t.toolId) || run.startedAt === null || instant(t.createdAt) < instant(run.startedAt)
                    ...
                    || terminal(run.run.state) && instant(t.updatedAt) > instant(run.finishedAt))
                    return bad("tool_conflict");
                toolMap.set(t.toolId, t);
            }
            const approvalIds = new Set(), nonces = new Set(), nativeIds = new Set();
            const unsettledCounts = new Map();
            for (const a of s.approvals) {
                ...
                if (!run || !lifecycle.checkApproval(a).valid || a.approval.sessionId !== cursor.sessionId || a.revision >= cursor.sequence
                    ...
                    || a.resolvedAt !== null && terminal(run.run.state) && instant(a.resolvedAt) > instant(run.finishedAt))
                    return bad("approval_conflict");
                approvalIds.add(a.approval.approvalId);
                nonces.add(a.approval.nonce);
                nativeIds.add(nativeKey);
                if (unsettled(a))
                    unsettledCounts.set(run.run.runId, (unsettledCounts.get(run.run.runId) ?? 0) + 1);
            }
            for (const run of runs.values()) {
                const count = unsettledCounts.get(run.run.runId) ?? 0;
                if (count > 32 || ["starting", "running"].includes(run.run.state) && count > 0)
                    return bad("approval_conflict");
            }
            for (const list of [s.usage, s.contexts]) {
                const runIds = new Set();
                for (const row of list) {
                    if (!bound(row) || runIds.has(row.runId))
                        return bad("entity_mismatch");
                    if ("lastCompaction" in row && row.lastCompaction && !bound(row, row.lastCompaction.createdAt))
                        return bad("state_time_conflict");
        ...
        function empty(cursor) {
            if (!contracts.validate("cursor", cursor).valid || cursor.sequence !== 0)
        ...
        async function applyBatch(prior, input) {
            ...
            if (!before.valid)
                return reset(before.code);
            if (encoded === null || !contracts.checkReplayBatch(input).valid)
                return reset("invalid_payload");
            const next = structuredClone(prior), batch = JSON.parse(encoded);
            const expectedCursor = structuredClone(next.cursor);
            if (batch.cursor.sessionId !== next.cursor.sessionId || batch.cursor.generation !== next.cursor.generation)
                return reset("cursor_mismatch");
            if (batch.afterCursor.sequence > next.cursor.sequence)
                return reset("sequence_gap");
            if (batch.events.length && batch.events[0].sequence <= next.identityFloor)
                return reset("replay_too_old");
            ...
            const byRun = new Map();
            for (const row of next.approvals) {
                const list = byRun.get(row.approval.runId) ?? [];
                list.push(row);
                byRun.set(row.approval.runId, list);
            }
            let writer = next.runs.find(row => !terminal(row.run.state)) ?? null, added = 0;
            function take(proposal) {
                if (proposal.kind === "reject")
                    throw new Error(proposal.code);
                return proposal.state;
            }
            function eventApply(event) {
                const now = instant(event.createdAt);
                if (!Number.isFinite(now) || next.updatedAt !== null && now < instant(next.updatedAt))
                    throw new Error("state_time_conflict");
                const at = new Date(now).toISOString(), runId = event.runId;
                const run = runId === null ? null : runs.get(runId) ?? null;
                if (event.type !== "session.created" && next.session === null)
                    throw new Error("session_conflict");
                if (runId !== null && event.type !== "run.starting" && run === null)
                ...
                const live = (started = true) => {
                    if (!run || terminal(run.run.state) || run.run.state === "orphaned" || started && run.startedAt === null)
                ...
                switch (event.type) {
                    ...
                    case "model.changed":
                        next.session = take(lifecycle.reduceSession(next.session, event, { expectedRevision: next.session?.revision ?? null, writer, profile: null }));
                        break;
                    case "run.starting":
                    case "launch_profile.locked":
                    case "run.started":
                    case "run.stopping":
                    case "run.orphaned":
                    case "run.resumed":
                    ...
                    case "run.interrupted": {
                        const updated = take(lifecycle.reduceRun(run, event, rc()));
                        runs.set(updated.run.runId, updated);
                        ...
                        if (terminal(updated.run.state)) {
                            ...
                            const cause = updated.run.state;
                            for (const m of messages.values())
                                if (m.runId === runId && m.status === "streaming") {
                                    m.status = "incomplete";
                                    m.terminalCause = cause;
                                    m.updatedAt = at;
                                }
                            for (const t of toolMap.values())
                                if (t.runId === runId && ["requested", "running"].includes(t.status)) {
                                    t.status = "incomplete";
                                    t.terminalCause = cause;
                                    t.updatedAt = at;
                                }
                        }
                        break;
                    ...
                    case "approval.acknowledged": {
                        ...
                        if (event.type === "approval.requested") {
                            ...
                            if (a.toolId !== null && toolMap.get(a.toolId)?.runId !== runId || nativeIds.has(key))
                        ...
                        nonces.add(row.approval.nonce);
                        const index = list.findIndex(a => a.approval.approvalId === id);
                        if (index < 0)
                            list.push(row);
                        else
                            list[index] = row;
                    ...
                    case "message.completed": {
                        ...
                        if (old && (old.runId !== runId || old.status !== "streaming" || old.role !== role))
                            throw new Error("message_conflict");
                        const m = old ?? { messageId: p.messageId, runId: runId, role, status: "streaming", text: "", thinking: "", createdAt: at, updatedAt: at, terminalCause: null };
                        if (event.type === "message.completed") {
                            m.text = event.payload.content;
                            m.status = "completed";
                        }
                        else {
                            const key = event.payload.channel === "text" ? "text" : "thinking";
                            m[key] += event.payload.delta;
                            if (points(m[key]) > 262144)
                                throw new Error("projection_capacity");
                        }
                        m.updatedAt = at;
                    ...
                    case "tool.failed": {
                        live();
                        const p = event.payload, old = toolMap.get(p.toolId);
                        if (event.type === "tool.requested") {
                            if (old)
                                throw new Error("tool_conflict");
                            toolMap.set(p.toolId, { toolId: p.toolId, runId: runId, name: event.payload.name, summary: event.payload.summary,
                                status: "requested", progress: "", output: null, error: null, createdAt: at, updatedAt: at, terminalCause: null });
                            break;
                        }
                        if (!old || old.runId !== runId || !["requested", "running"].includes(old.status))
                            throw new Error("tool_conflict");
                        if (event.type !== "tool.failed" && list.some(a => a.approval.toolId === p.toolId && (a.approval.status !== "approved" || a.nativeAcknowledgement === null)))
                            throw new Error("approval_conflict");
                        if (event.type === "tool.started") {
                            ...
                            old.status = "failed";
                            old.error = structuredClone(event.payload.error);
                        }
                        else {
                            ...
                            if (event.type === "tool.completed") {
                                ...
                                old.output = event.payload.output;
                            }
                            else {
                                old.progress += event.payload.text;
                                if (points(old.progress) > 262144)
                                    throw new Error("projection_capacity");
                            }
                        }
                        old.updatedAt = at;
                    ...
                    case "usage.updated": {
                        ...
                        if (old && ["inputTokens", "outputTokens", "cachedTokens"].some(key => event.payload[key] < old[key]))
                            throw new Error("usage_regression");
                        usage.set(runId, { runId: runId, updatedAt: at, inputTokens: event.payload.inputTokens, outputTokens: event.payload.outputTokens, cachedTokens: event.payload.cachedTokens });
                        break;
                    }
                    ...
                    case "context.compacted": {
                        ...
                        contexts.set(runId, event.type === "context.updated" ? { runId: runId, updatedAt: at, usedTokens: event.payload.usedTokens,
                            ...
                            limitTokens: old?.limitTokens ?? null, lastCompaction: { beforeTokens: event.payload.beforeTokens, afterTokens: event.payload.afterTokens, createdAt: at } });
                        break;
                    }
            ...
            try {
                for (const event of batch.events) {
                    ...
                    if (event.sequence <= expectedCursor.sequence) {
                        ...
                        if (!old || old.eventId !== event.eventId || old.digest !== digest)
                            return reset("event_conflict");
                        continue;
                    }
            ...
            if (!added)
                return { kind: "duplicate", expectedCursor };
            next.cursor = structuredClone(batch.cursor);
            if (next.identities.length > WINDOW)
                next.identities.splice(0, next.identities.length - WINDOW);
            next.identityFloor = next.cursor.sequence - next.identities.length;
            next.runs = Array.from(runs.values());
            next.approvals = Array.from(approvals.values());
            next.messages = Array.from(messages.values());
            next.tools = Array.from(toolMap.values());
            next.usage = Array.from(usage.values());
            next.contexts = Array.from(contexts.values());
            const checked = checkState(next);
            return checked.valid ? { kind: "apply", expectedCursor, state: next } : reset(checked.code);
        }
        async function sealSnapshot(input) {
            const checked = checkState(input);
            if (!checked.valid)
                return { kind: "reject", code: checked.code };
            const canonical = canonicalJSON(input);
            const state = JSON.parse(canonical);
            try {
                return { digestVersion: "sha256-sorted-json-v1", digest: await hash("snapshot", canonical), state };
            }
            catch {
                return { kind: "reject", code: "digest_unavailable" };
            }
        }
        async function restoreSnapshot(current, input, context) {
            if (!checkState(current).valid || canonicalJSON(input, LIMIT + 1024) === null || !contracts.validate("projectionSnapshot", input).valid
                || !context || !contracts.validate("cursor", context.expectedCursor).valid || !contracts.validate("id", context.targetGeneration).valid)
                return reset("invalid_payload");
            const expectedCursor = structuredClone(current.cursor), snapshot = structuredClone(input);
            if (!sameCursor(expectedCursor, context.expectedCursor))
                return reset("stale_snapshot");
            ...
            if (target.sessionId !== expectedCursor.sessionId || target.generation !== context.targetGeneration
                || target.generation === expectedCursor.generation && target.sequence < expectedCursor.sequence)
                return reset("cursor_mismatch");
            const check = checkState(snapshot.state);
            if (!check.valid)
                return reset(check.code);
            try {
                if (await hash("snapshot", canonicalJSON(snapshot.state)) !== snapshot.digest)
                    return reset("snapshot_corrupt");
            }
            catch {
                return reset("digest_unavailable");
            }
        ...
        function createReplica(initial) {
            if (!checkState(initial).valid)
                throw new Error("invalid_projection");
            let state = structuredClone(initial), revision = 0;
            function publish(result, expected) {
                if (revision !== expected)
                    return reset("stale_projection");
                if (result.kind === "apply") {
                    if (revision === Number.MAX_SAFE_INTEGER)
                        return reset("revision_overflow");
            ...
            return Object.freeze({
                ...
                snapshotRequest: (targetGeneration) => {
                    if (!contracts.validate("id", targetGeneration).valid)
                ...
                apply: async (input) => {
                    ...
                    return publish(await applyBatch(state, input), expected);
                },
                restore: async (input, request) => {
                    if (!request || request.localRevision !== revision)
                        return reset("stale_snapshot");
                    const expected = revision;
                    return publish(await restoreSnapshot(state, input, request), expected);
                },
            });
        }
        return Object.freeze({ empty, checkState, applyBatch, sealSnapshot, restoreSnapshot, createReplica });
    }
    StepsembleProjection.create = create;
})(StepsembleProjection || (StepsembleProjection = {}));
if (typeof module !== "undefined")
    module.exports = StepsembleProjection;
