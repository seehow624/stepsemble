...
(function (StepsembleCodexHistoryView) {
    ...
    function createModel(deps) {
        ...
        async function load(mode, target, interruptPrevious = true) {
            ...
            try {
                ...
                const selection = { offset, limit: StepsembleCodexHistoryView.LIMITS.pageRecords };
                ...
                const readOptions = () => ({ page: selection, signal: current.controller.signal, ...(version === undefined ? {} : { version }),
                    ...(readProfile ? { profile: readProfile } : readStructured ? { structured: true } : {}) });
    ...
    function create(deps) {
        ...
        jumpInput.dataset.action = "recordNumber";
        i18n.bind(jumpInput, "codexJumpLabel", {}, "aria-label");
        jumpLabel.append(copy("span", "codexJumpLabel"), jumpInput);
        ...
        jumpForm.addEventListener("submit", event => {
            ...
            if (jumpInput.value.trim() && Number.isSafeInteger(n) && n > 0)
                void model.jump(n - 1);
        });
        ...
        function render() {
            ...
            for (const row of r.records) {
                ...
                if (!annotation && completeText) {
                    ...
                    try {
                        const v = JSON.parse(row.rawText), p = v.payload;
                        if (v.type === "response_item" && p?.type === "message" && ["assistant", "user"].includes(p.role))
                            role = p.role;
                        if (v.type === "event_msg" && ["agent_message", "user_message"].includes(p?.type))
                            role = p.type === "agent_message" ? "assistant" : "user";
                    }
                    ...
                    if (role) {
                        ...
                        if (role === "assistant")
                            ...
                            heading.replaceChildren(copy("span", "you"));
                        heading.append(el("span", ` · ${row.recordIndex + 1}`, "history-footnote"));
                    }
                }
                const text = excerpt(row), preview = el("p", completeText ? text : text.slice(0, StepsembleCodexHistoryView.LIMITS.previewUnits), "history-message-text");
                preview.tabIndex = 0;
                ...
                if (!completeText && text.length > StepsembleCodexHistoryView.LIMITS.previewUnits)
