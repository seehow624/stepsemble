...
Plan1.78已另提供[本機多群組管理](history-owner-management.md)：
`node scripts/history-manage.mjs --lang zh-Hant`可檢視／新增／替換／編輯／移除，
以新的候選檔保留原設定。下方「不合併多群組」只描述這個單來源建立精靈，
不是新版完全無法管理群組；兩者均不熱載入或啟用正式Host。
