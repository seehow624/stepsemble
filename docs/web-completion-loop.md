...
| C2 各 Agent 原生歷史 | 各自固定版本 API／格式、native ID/name、主／subagent 範圍、完整歷史與原生名稱驗證；未知版本／來源有清楚狀態 | Plan1.68接通SQL A→bytes A→parser→SQL B→bytes B，共用單permit/總期限，雙版本/取消/cleanup gate；本機922/0fail、reader187、Rust21/28、真named鏈90reader/parser/35真SHM/max2/remaining0，六mutation/五實際cancel通；CI待核。非原子snapshot/final title，授權/discovery/registry/CodexHTTPWeb、cold/壓縮/其他adapter仍待，C2未完成 |
