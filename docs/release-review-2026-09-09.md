...
狀態：**72h已核對通過；發布審查找出兩項待修問題，候選未升為 stable，沒有部署或重新授權私人來源。**

**新增阻擋問題**：審查發現Pi＋worktree開啟時，Host回原生`sid`，前端卻要求
generic task ID，會顯示失敗並可能留下已啟動的Pi程序；正在修正原生連線分流與
late-response清理、補owned回歸。此項未驗證完成前，不可發布候選為正式版。

本文件是今晚的單一收尾入口。功能實作與證據截至產品工程
`1b37173e8e8569e3c0bb18d807bff5ab20ff7a67`、browser runner 修正
`2beab6853497da210edc645200a1144095e31618`；本輪另補發布分流保護，必須獨立驗證。
...
1. **發布通道保護**：本輪審查發現原workflow會對所有`v*` tag直接建立普通Release，
   ...
   fail-closed；actionlint通。新提交仍須必要CI。沒有建立tag／Release或切換更新來源。
