...
Stepsemble supports the latest stable release. Automatic updates follow GitHub
Releases, verify the published SHA-256 checksum, and wait for active agent work to
...
Stepsemble is a single-user, self-hosted interface. Keep the Node service bound
...
headers. New `STEPSEMBLE3` pairing uses a reviewed, five-minute capability to
issue a separate revocable peer credential; incoming hashes and outgoing
credentials are stored in `~/.config/stepsemble/device-trust.json` with mode
...
once, stored only as a SHA-256 hash in `~/.config/stepsemble/tokens.json`
...
coding-agent account or project permission boundary.
...
```bash
gh attestation verify stepsemble-vX.Y.Z.tar.gz --repo seehow624/stepsemble
```
