...
function readBody(req, signal, maxBytes = LIMITS.requestBytes, maxChunks = LIMITS.requestChunks) {
  return new Promise((resolve, reject) => {
    ...
    const data = chunk => {
      ...
      size += chunk.length;
      if (size > maxBytes || ++count > maxChunks) return finish(error("history_body_too_large", 413));
      chunks.push(chunk);
    };
    const end = () => {
      try {
        ...
        if (!object(value) || canonicalJSON(value, maxBytes) === null) return failed();
        finish(null, value);
      } catch { failed(); }
    };
...
function validPaginatedResolutionRequest(value) {
  const hasVersion = Object.hasOwn(value ?? {}, "version");
  return exact(value, ["bindingId", "generation", "requestId", "selectedRolloutId", "entries", ...(hasVersion ? ["version"] : [])])
