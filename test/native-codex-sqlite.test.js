"use strict";
const baseTest = require("node:test"), assert = require("node:assert/strict"), path = require("node:path");
...
for (const withContext of [false, true]) {
  ...
  test("v4 detached input and bounded metadata publish only after actual close, never stdout or exit", async t => {
    const h = harness(t), r = fixture.request(), p = h.helper[method](r), c = h.children[0];
    r.source.sqliteRoot = path.resolve("changed"); r.expectedRoot.inode = "9";
    assert.equal(c.job.protocolVersion, version); assert.equal(c.job.nativeVersion, wire.VERSION); assert.deepEqual(c.job.source, fixture.request().source);
  ...
  test("independent SQLite root/UUID/version/identity and strict options reject without invoking getters or spawning", async t => {
    ...
    for (const r of [null, {}, getter, { ...fixture.request(), nativeVersion: "latest" }, { ...fixture.request(), env: {} },
      ...
      assert.equal((await h.helper[method](r)).code, "invalid_source_input");
    assert.equal((await h.helper[method](fixture.request(), { get signal() { calls++; return null; } })).code, "invalid_source_signal");
  ...
  test("SQLite frame rejects mixed nonce/version/root/thread, checksums, cleanup and authority claims", async t => {
    for (const mutate of [v => { v.expectedRoot.inode = "9"; }, v => { v.threadId = fixture.id.replace("aaaa", "ffff"); },
      ...
      const h = harness(t), p = h.helper[method](fixture.request()), f = fixture.packet(); mutate(f.header); h.children[0].reply(f);
      assert.equal((await p).code, "source_worker_protocol");
    }
    for (const change of [v => ({ ...v, nonce: "a" }), v => ({ ...v, protocolVersion: 3 }), v => ({ ...v, extra: true })]) {
      const h = harness(t), p = h.helper[method](fixture.request()); h.children[0].reply(fixture.packet(), change); assert.equal((await p).code, "source_worker_protocol");
    }
  });
  test("SQLite payload exact fields, text limits, pinned engine, file identities and I/O budgets fail closed", () => {
    ...
    const b = fixture.body(); b.observation.fields = null; if (withContext) b.observation.nameContext = null; b.shmMappingsClosed = 0; b.mappedShmBytes = 0;
  ...
  test("v4 shares v1/v2/v3 single flight, actual-close cancellation and permanent cleanup quarantine", async t => {
    const h = harness(t), controller = new AbortController(), p = h.helper[method](fixture.request(), { signal: controller.signal }), c = h.children[0]; c.hold = true;
    const root = path.resolve("claude"), expectedRoot = fixture.request().expectedRoot;
    const peers = [() => h.helper[withContext ? "readCodexMetadata" : "readCodexNameContext"](fixture.request()),
      () => h.helper.read({ source: { projectsRoot: root, projectKey: "owned", sessionId: fixture.id }, expectedRoot }),
    ...
    assert.equal((await h.helper[method](fixture.request())).code, "source_service_quarantined"); assert.equal(h.children.length, 1);
  });
  test("partial/oversized/trailing frames and stderr contain no metadata; sanitized new SQLite refusals are preserved", async t => {
    for (const mode of ["partial", "trailing", "oversized", "stderr", "exit"]) {
      const h = harness(t), p = h.helper[method](fixture.request()), c = h.children[0], bytes = fixture.frame(c.job);
    ...
    for (const code of ["source_database_unsupported", "source_database_unavailable", "source_busy", "source_cancelled"]) {
      const h = harness(t), p = h.helper[method](fixture.request()); h.children[0].reply({ header: { kind: "source_unavailable", code }, payload: Buffer.alloc(0) });
  ...
  test("Windows and pre-aborted metadata reads do not launch a child or promote platform capability", async t => {
    const win = harness(t, { platform: "win32" }); assert.equal((await win.helper[method](fixture.request())).code, "source_platform_unsupported"); assert.equal(win.children.length, 0);
