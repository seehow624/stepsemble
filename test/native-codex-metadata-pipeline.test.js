...
const { createCodexMetadataPipeline } = require("../protocol/native/codex/metadata-pipeline");
const { createReaderAdmission } = require("../protocol/native/claude/history-reader-admission");
const fixture = require("../protocol/native/codex/sqlite-fixture.cjs");
...
function harness(t, config = {}) {
  ...
  const pipeline = createCodexMetadataPipeline({ helperPath: process.execPath, admission, platform: "linux", deadlineMs: config.deadlineMs ?? 2000,
    cleanupMs: 20, createHelper() {
      ...
      h.readCodexMetadata = (input, { signal }) => {
...
test("shared admitted capture detaches input and interprets legacy/paginated/missing rows without final native title claims", async t => {
  ...
  assert.equal(a.kind, "codex_sqlite_name_capture"); assert.equal(a.metadata.candidate, "原生候選 🐾");
  ...
  assert.equal((await complete(h, {}, b)).metadata.candidate, "分頁名稱"); b.observation.fields = null;
...
test("all metadata consumers share two physical permits and never queue or create replacement helpers", async t => {
  ...
  assert.equal((await p).kind, "codex_sqlite_name_capture"); assert.equal((await q).kind, "codex_sqlite_name_capture");
...
test("selected-field fence ignores counters but rejects changed fields and file identity before name publication", async t => {
  ...
  assert.equal((await complete(h, { expectedVersion: a.source }, b)).kind, "codex_sqlite_name_capture");
...
test("closing this consumer does not close another shared consumer", async t => {
  ...
  assert.equal(admission.status().closed, false); b.helpers[0].finish(); assert.equal((await p).kind, "codex_sqlite_name_capture");
});
...
test("constructor requires genuine caller-owned admission and strict helper lifecycle; Windows stays unsupported", async () => {
  ...
  for (const bad of [{ ...options, admission: undefined }, { ...options, admission: { ...options.admission } }, { ...options, env: {} },
    ...
    assert.throws(() => createCodexMetadataPipeline(bad), /invalid_/);
  const p = createCodexMetadataPipeline({ ...options, platform: "win32" }); assert.equal((await p.read(fixture.request())).code, "source_platform_unsupported");
