...
test("Codex structured mode cannot invoke a Claude source or claim its registration", async () => {
  ...
  assert.equal((await h.registry.observe("alice", request(registration, { profile: "codex_validated_page_v1" }))).code, "invalid_history_request");
  assert.equal((await h.registry.metadata("alice", { ...scope(registration), requestId, profile: "codex_validated_page_v1" })).code, "invalid_history_request");
