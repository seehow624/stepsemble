...
test("the workspace shell sizes the brand mark without replacing its artwork", () => {
  ...
  // The empty pane shows the same approved artwork. Only the forced-colour
  // fallback may mask the glyph, and it must live in its own block.
  const slotStart = css.indexOf(".workspace-slot::before {");
  assert.ok(slotStart > -1, "the empty pane carries the brand mark");
  const slot = css.slice(slotStart, css.indexOf("}", slotStart));
  assert.match(slot, /background: #121212 url\("\/icon-512\.png"\)/);
  assert.match(slot, /border-radius: 24%/);
