...
여러 사람이나 기기가 한 호스트를 함께 사용하는 경우, 설치 시의 마스터 토큰으로 **Settings → Access tokens**를 열어 라벨이 있는 토큰을 발급할 수 있습니다. 토큰은 한 번만 표시되고 개별적으로 취소할 수 있으며, 서버에는 SHA-256 해시만 `~/.config/pi-harbor/tokens.json`(권한 `600`)에 저장됩니다. 별도의 Pi 계정이나 프로젝트 권한을 만드는 기능은 아닙니다.
