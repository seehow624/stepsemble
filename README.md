...
**Pi Harbor** is an open-source, mobile-first web client for the [Pi coding
...
multiple Pi Harbor computers from a phone or desktop browser.
...
Pi Harbor is self-hosted. The repository contains only application code and
...
- Multiple Pi Harbor computers with aliases, health checks, port settings, and
...
Run these commands on each computer that will host Pi Harbor:
...
Keep Pi Harbor bound to loopback. Put Tailscale Serve or another authenticated
...
Pi Harbor reads sessions and provider settings from the normal Pi directories
under the configured home. Those directories stay outside this repository.

## Multiple computers

Install and run one local Pi Harbor instance on every computer that should be
...
Pi Harbor can check the public repository on a schedule, download a verified
...
Pi Harbor intentionally has no runtime npm dependencies:
