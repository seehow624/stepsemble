...
複数の人やデバイスでホストを共有する場合は、インストール時のマスタートークンで **Settings → Access tokens** を開き、ラベル付きのトークンを発行できます。トークンは一度だけ表示され、個別に取り消せます。サーバーには SHA-256 ハッシュだけが `~/.config/pi-harbor/tokens.json`（権限 `600`）に保存されます。別の Pi アカウントやプロジェクト権限を作る機能ではありません。
